...
lazy_static! {
    static ref SNOW: SnowflakeIdWorker = SnowflakeIdWorker::new(20);
}
...
impl SnowflakeIdWorkerInner {
       ...
       fn next_id(&mut self) -> u64 {
               ...
               if timestamp < self.last_timestamp {
                       panic!("{}",format!("Clock moved backwards.  Refusing to generate id for {} milliseconds", self.last_timestamp - timestamp));
                   }
           ...
           ((((timestamp - START_TIME) << TIMESTAMP_LEFT_SHIFT) as u64) | (self.worker_id << WORKER_ID_SHIFT) | self.sequence)&(!(1u64<<63))
           }
       // 计算一个大于上一次时间戳的时间戳
