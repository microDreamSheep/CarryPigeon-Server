...
use std::{cell::SyncUnsafeCell, sync::OnceLock};

pub static MYSQL_POOL: OnceLock<RBatis> = OnceLock::new();
pub static REDIS_POOL: OnceLock<SyncUnsafeCell<MultiplexedConnection>> = OnceLock::new();
...
pub async fn init_pool() {
    ...
    match redis::Client::open("redis://localhost:6379/0") {
        Ok(v) => match v.get_multiplexed_async_connection().await {
            Ok(v) => match REDIS_POOL.set(v.into()) {
                Ok(_) => {
