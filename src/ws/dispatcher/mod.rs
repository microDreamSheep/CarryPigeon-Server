...
// 需要引入的方法
use crate::controller::chat::message::chat_sender_controller;

/**
请求分发
*/
pub async fn ws_dispatcher(request_data: WebSocketRequest) -> WebSocketResponse {
    let handler = WS_DISPATCHER.get().unwrap().dispatch(&request_data.route).await;
...
impl WebSocketDispatcher {
    pub fn new() -> WebSocketDispatcher {
        ...
        result.attach_route("chat", |x| Box::pin(chat_sender_controller(x)));
        result
    }
    /**
    获取路径的引用
    */
    pub async fn dispatch(
        &self,
        path: &str,
    ) -> Option<&Arc<dyn Fn(Value) -> Pin<Box<dyn Future<Output = WebSocketResponse> + Send >> + Send + Sync>>
    {
