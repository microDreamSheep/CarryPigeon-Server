...
pub async fn ws_dispatcher(request_data: WebSocketRequest) -> WebSocketResponse {
    let handler = WS_DISPATCHER.get().unwrap().dispatch(&request_data.route);
    return match handler {
        None => WEBSOCKET_RESPONSE_ROUTE_ERROR.get().unwrap().clone(),
        Some(handler) => handler.call((request_data.data,)),
    };
}
...
impl WebSocketDispatcher {
    ...
    获取路径的引用
    */
    pub fn dispatch(&self, path: &str) -> Option<&fn(Value) -> WebSocketResponse> {
    ...
    */
    pub fn attach_route(
        mut self,
        route: &str,
        handler: fn(Value) -> WebSocketResponse,
    ) -> WebSocketDispatcher {
